...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=4, in_chans=3,
                 embed_dim=96, norm_layer=None, tubelet_size=1, use_conv_embed=False):
        super().__init__()
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([x0, x1, x2, x3, x4, x5, x6, x7], dim=1)  # B, 8*C, T/2, H/2, W/2

        # Apply normalization and reduction
        x = x.permute(0, 2, 3, 4, 1).contiguous()  # B, T/2, H/2, W/2, 8*C
        x = self.norm(x)
        x = self.reduction(x)
        x = x.view(B, T // 2, H // 2, W // 2, -1)
        x = x.permute(0, 4, 1, 2, 3).contiguous()  # B, C', T/2, H/2, W/2
...
class VideoFocalNet(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_postln_in_modulation=False, normalize_modulator=False,
                 tubelet_size=1, num_frames=8, use_conv_embed=False, **kwargs):
        super().__init__()
        ...
        self.mlp_ratio = mlp_ratio
        self.tubelet_size = tubelet_size
        self.num_frames = num_frames // tubelet_size
        self.use_conv_embed = use_conv_embed
        ...
        self.patch_embed = PatchEmbed(
            ...
            norm_layer=norm_layer if kwargs.get('patch_norm', True) else None,
...
def videofocalnet_tiny(pretrained=False, **kwargs):
    kwargs.setdefault('num_frames', 8)
