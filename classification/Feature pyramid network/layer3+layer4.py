...
class VideoFocalNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.lateral_layer3 = nn.Linear(embed_dim[3], self.fuse_dim)
    ...
    def forward_features(self, x):
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if idx == 2:  # Second stage (indexing starts from 0)
                output_stage3 = x
                H3, W3 = H, W
        ...
        x = torch.flatten(x, 1)
        return x, output_stage3, H3, W3, output_stage4, H4, W4
    ...
    def forward(self, x):
        ...
        x, output_stage3, H3, W3, output_stage4, H4, W4 = self.forward_features(x)
        B = x.shape[0]  # Batch size

        # Check if outputs are captured
        if output_stage3 is not None and output_stage4 is not None:
            # Project outputs to the same channel dimension
            output_stage3 = self.lateral_layer3(output_stage3)  # [B, L2, fuse_dim]
            ...
            # Upsample output_stage4 to match spatial dimensions of output_stage3
            output_stage4_upsampled = F.interpolate(output_stage4, size=(H3, W3), mode='bilinear', align_corners=False)

            # Fuse the outputs (addition)
            fused_output = output_stage3 + output_stage4_upsampled
