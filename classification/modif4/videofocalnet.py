...
class CrossFrameAttention(nn.Module):
    ...
    def forward(self, x):
        B, T, N, C = x.shape  # B: batch size, T: num frames, N: num patches, C: channels
...
class SpatioTemporalFocalModulation(nn.Module):
    ...
    def forward(self, x):
        ...
        Args:
            ...
            B: Batch size, T: Temporal dimension, H: Height, W: Width, C: Channels
        """
        B, T, H, W, C = x.shape

        # Réorganisation des frames pour traitement global
        ...
        x = self.cross_frame_attn(q)
        
        # Revenir à la disposition d'origine pour la modulation spatiale
        x = rearrange(x, '(b h w) t c -> b t h w c', h=H, w=W)
        ctx = rearrange(ctx, '(b h w) t c -> b t h w c', h=H, w=W)
        self.gates = rearrange(self.gates, '(b h w) t c -> b t h w c', h=H, w=W)

        # Agrégation contextuelle spatiale
        ctx_all = 0
        for l in range(self.focal_level):
            ctx = rearrange(ctx, 'b t h w c -> (b t) c h w')
            ctx = self.focal_layers[l](ctx)
            ctx = rearrange(ctx, '(b t) c h w -> b t h w c', t=T)
            ctx_all = ctx_all + ctx * self.gates[:, :, :, :, l:l + 1]
        ctx_global = self.act(ctx.mean(2, keepdim=True).mean(3, keepdim=True))
        ctx_all = ctx_all + ctx_global * self.gates[:, :, :, :, self.focal_level:]

        # Modulation spatiale
        self.modulator = self.h(ctx_all.view(-1, C, H, W))

        # Application de la modulation sur q
        x = rearrange(q, '(b h w) t c -> b t h w c', h=H, w=W)
        x = rearrange(x * self.modulator.view(B, 1, C, H, W), 'b t c h w -> (b t) h w c')
