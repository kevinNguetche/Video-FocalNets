...
class SpatioTemporalFocalModulation(nn.Module):
    def __init__(self, dim, focal_window, focal_level, focal_factor=2, bias=True, proj_drop=0.,
        ...
        # Create alternating focal levels for spatial and temporal layers
        for k in range(self.focal_level):
            ...
            self.temporal_layers.append(
                nn.Sequential(
                    nn.Conv1d(dim, dim, kernel_size=kernel_size, stride=1, padding=kernel_size//2, bias=False), 
                    nn.GELU(),
                )
            )
...
class VideoFocalNetBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply spatio-temporal modulation alternately
        x = self.norm1(x)
        x = x.view(B, H, W, C)
