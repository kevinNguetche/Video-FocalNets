...
class CrossFrameAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        # Shared projection for qkv
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)

        # Separate projections for spatial and temporal attentions
        self.proj_spatial = nn.Linear(dim, dim)
        self.proj_temporal = nn.Linear(dim, dim)

        # Final projection after concatenation
        self.final_proj = nn.Linear(dim * 2, dim)
    ...
    def forward(self, x):
        ...
        # Common qkv calculation
        qkv = self.qkv(x).reshape(B, T, N, 3, self.num_heads, C // self.num_heads)

        ### Spatial Attention ###
        qkv_spatial = qkv.permute(3, 0, 4, 1, 2, 5)  # Shape: (3, B, num_heads, T, N, head_dim)
        q_s, k_s, v_s = qkv_spatial[0], qkv_spatial[1], qkv_spatial[2]  # Each: (B, num_heads, T, N, head_dim)
        ...
        ### Temporal Attention ###
        qkv_temporal = qkv.permute(3, 0, 4, 2, 1, 5)  # Shape: (3, B, num_heads, N, T, head_dim)
        q_t, k_t, v_t = qkv_temporal[0], qkv_temporal[1], qkv_temporal[2]  # Each: (B, num_heads, N, T, head_dim)
        ...
        x = torch.cat((x_s, x_t), dim=-1)  # Concatenate along the channel dimension (C)
        x = self.final_proj(x)  # Project back to the original dimension
        x = self.proj_drop(x)

        return x  # Final output after concatenation of attentions
