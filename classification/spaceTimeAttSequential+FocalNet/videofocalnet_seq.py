...
class CrossFrameAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        ### Spatial Attention ###
        qkv_s = self.qkv_spatial(x).reshape(B, T, N, 3, self.num_heads, C // self.num_heads)
        qkv_s = qkv_s.permute(3, 0, 4, 1, 2, 5)  # Shape: (3, B, num_heads, T, N, head_dim)
        q_s, k_s, v_s = qkv_s[0], qkv_s[1], qkv_s[2]  # Each: (B, num_heads, T, N, head_dim)
        ...
        ### Temporal Attention on Data from Spatial Attention ###
        qkv_t = self.qkv_temporal(x_s).reshape(B, T, N, 3, self.num_heads, C // self.num_heads)
        qkv_t = qkv_t.permute(3, 0, 4, 2, 1, 5)  # Shape: (3, B, num_heads, N, T, head_dim)
        q_t, k_t, v_t = qkv_t[0], qkv_t[1], qkv_t[2]  # Each: (B, num_heads, N, T, head_dim)
        ...
        return x_t  # Final output after both sequential attentions
