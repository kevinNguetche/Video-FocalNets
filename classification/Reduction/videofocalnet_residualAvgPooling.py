...
class TemporalDownsample(nn.Module):
    def __init__(self, channels, kernel_size=2, stride=2):
        ...
        self.residual_proj = nn.Conv1d(channels, channels, kernel_size=1, stride=stride, padding=0, bias=False)
        self.residual_bn = nn.BatchNorm1d(channels)
    ...
    def forward(self, x):
        ...
        residual = self.residual_proj(x)  # [B*L, C, T_reduced]
        residual = self.residual_bn(residual)
        x = x_proj + residual  # [B*L, C, T_reduced]
        x = F.relu(x)
...
class VideoFocalNet(nn.Module):
    ...
    def __init__(self, 
        ...
        if 1 < self.num_layers:
            temporal_embed_dim = embed_dim[2]  # Nombre de canaux à la layer idx==1
    ...
    def forward_features(self, x):
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if idx == 1 and self.temporal_downsample is not None:
                ...
                # Appliquer l'AvgPool1d + Projection
